...
const fetchPerms = async () => {
  if (props.permissions?.length) {
    allPerms.value = props.permissions
    return
  }
  try {
...
const groupedPerms = computed(() => {
  ...
  const filtered = allPerms.value.filter(p =>
...
const togglePerm = (id: number) => {
  const i = form.value.permissions.indexOf(id)
  i === -1 ? form.value.permissions.push(id) : form.value.permissions.splice(i, 1)
}

const toggleGroup = (perms: Permission[]) => {
  const ids = perms.map(p => p.id)
  const all = ids.every(id => form.value.permissions.includes(id))
  if (all) {
...
const isGroupSelected = (perms: Permission[]) =>
  perms.every(p => form.value.permissions.includes(p.id))

const isGroupPartial = (perms: Permission[]) => {
  const s = perms.filter(p => form.value.permissions.includes(p.id))
...
const submit = async () => {
  ...
  errors.value = {}
  try {
    const permNames = form.value.permissions
  ...
  <div class="container mx-auto py-6 px-4 sm:px-6 lg:px-8 space-y-6 max-w-4xl">
    ...
    <form @submit.prevent="submit" class="space-y-6">
      ...
      <div class="rounded-xl border bg-card shadow-sm">
        <div class="border-b p-4 flex items-center justify-between">
          <div>
            ...
            <p class="text-sm text-muted-foreground">{{ form.permissions.length }}/{{ allPerms.length }} selected</p>
          </div>
          <div class="flex gap-2 text-sm">
            <button type="button" @click="selectAll" class="text-primary hover:underline">Select All</button>
            <span class="text-muted-foreground">|</span>
            <button type="button" @click="deselectAll" class="text-primary hover:underline">Deselect All</button>
          </div>
        </div>
        <div class="p-4">
          <!-- Search -->
          <div class="relative mb-4">
            ...
            <input v-model="permSearch" type="text" placeholder="Search..." class="h-10 w-full rounded-lg border bg-background pl-10 pr-3 text-sm" />
          </div>

          <!-- Empty -->
          <div v-if="!Object.keys(groupedPerms).length" class="py-8 text-center text-muted-foreground">
            {{ permSearch ? 'No matches' : 'No permissions. Run: php artisan roles:sync' }}
          </div>
            ...
            <div v-for="(perms, group) in groupedPerms" :key="group" class="rounded-lg border">
              ...
              <div class="grid gap-2 p-4 sm:grid-cols-2 md:grid-cols-3">
                <label
                  ...
                  :key="p.id"
                  :class="[
                    'flex items-center gap-2 rounded-lg border p-3 cursor-pointer hover:bg-muted/50',
                    form.permissions.includes(p.id) ? 'bg-primary/5 border-primary/50' : ''
                  ]"
                >
                  <input
                    type="checkbox"
      ...
      <div class="flex justify-end gap-3">
        <a :href="uiUrl('/roles')" class="h-10 px-4 rounded-lg border inline-flex items-center text-sm font-medium hover:bg-muted">
          Cancel
        </a>
        <button
          type="submit"
